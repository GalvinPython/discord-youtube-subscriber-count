		...
		try {
			...
			if (isMilestone && data.new.subscribers >= 100_000) {
				...
				const quickie = QuickMakeEmbed(
					...
					const timeTillCool = Math.floor((Date.now()/1000)+((milestoneAPICount-currentAPICount)/(parseInt(data.new.sub_rate)/86400)));
					if(typeOfMilestone)
					quickie.addFields({
						name:
				...
				const quickie = QuickMakeEmbed(
					{
						...
						title: `🔥 Fast channel sub count update for ${channel.title}`.slice(
							0,
							255,
						),
